(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/framework/utils/af-utils.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '3efe0YXqcxBCpCyH1OeMsZo', 'af-utils', __filename);
// framework/utils/af-utils.js
...
var wxShareOpen = require('../config/wx/wxShare.js');
var wxVideoAdOpen = require('../config/wx/wxVideoAd.js');
var wxBannerAdOpen = require('../config/wx/wxBannerAd.js');
var qqShareOpen = require('../config/qq/qqShare.js');
var qqVideoAdOpen = require('../config/qq/qqVideoAd.js');
var qqBannerAdOpen = require('../config/qq/qqBannerAd.js');
...
var utils = {
    getSchemeParam: function getSchemeParam(url, scheme) {
        var prefix = scheme + '://';
    ...
    // 是否是全面屏
    isAllScreen: function isAllScreen() {
    ...
    gotoGameScene: function gotoGameScene(sceneName, params) {
        AF.gotoScene(sceneName, params);
    },
    isZipFile: function isZipFile(filename) {
        return '' !== AF.util.getZipFilename(filename);
    },
    getZipFilename: function getZipFilename(filename) {
    ...
    getFramName: function getFramName(filename) {
        var leng = filename.length;
        var index = filename.lastIndexOf("/");
        if (index >= 1 && index < leng) {
    ...
    deepCopy: function deepCopy(obj, c) {
        var c = c || {};
        for (var key in obj) {
    ...
    spliceByChar: function spliceByChar(str, s) {
        var nums = [];
        while (true) {
            var i = str.indexOf(s);
            if (-1 === i) {
    ...
    replaceAToB: function replaceAToB(str, a, b) {
        b = b || '';
        var re = new RegExp(a, "g");
        return str.replace(re, b);
    },

    parseNums: function parseNums(str) {
        //str {num1,mun2}
    ...
    createPrefabNode: function createPrefabNode(prefab) {
        return cc.instantiate(prefab);
    },

    printTime: function printTime(key) {
    ...
    getCurrTimeOfSecond: function getCurrTimeOfSecond() {
        return Math.round(this.getCurrTime() / 1000);
    },

    queryObject: function queryObject(key) {
    ...
    getMinAndSec: function getMinAndSec(sec) {
        var minute = 0;
    ...
    //time的单位秒
    formatPastTime: function formatPastTime(seconds) {
    ...
    //time的单位秒
    formatTimeToHHMMSS: function formatTimeToHHMMSS(seconds) {
        if (seconds > 60 * 60 * 24) {
            return Math.floor(seconds / (60 * 60 * 24)) + "天";
        }
        var ss = 0;
    ...
    openDialogOnce: function openDialogOnce(name, params, callback) {
    ...
    sortFunc: function sortFunc(arr, key) {
    ...
    isShareOpen: function isShareOpen() {
        //分享开关打开
    ...
    isBannerAdOpen: function isBannerAdOpen() {
    ...
    getAdQuery: function getAdQuery(q) {
        var sb = [];
        for (var k in q) {
            sb.push(k + '=' + q[k]);
        }
    ...
    showUserInfoButton: function showUserInfoButton(left, top, width, height, cb) {
        if (AF.platform.isWxApp()) {
            var systemInfo = AF.platform.getSystemInfo();
            AF.wxHelper.showInfoButton(Math.floor(left / cc.winSize.width * systemInfo.windowWidth), Math.floor(top / cc.winSize.height * systemInfo.windowHeight), Math.floor(width / cc.winSize.width * systemInfo.windowWidth), Math.floor(height / cc.winSize.height * systemInfo.windowHeight), cb);
        }
    },

    hideUserInfoButton: function hideUserInfoButton() {
        if (AF.platform.isWxApp()) {
    ...
    //分享
    shareAppMessage: function shareAppMessage(scene, success, fail, queryData, type, content) {
        ...
        if (AF.platform.isWxPhone()) {
            var onFetchShareStatus = function onFetchShareStatus() {
    ...
    isVideoAdOpen: function isVideoAdOpen() {
    ...
    //看视频
    showVideoAd: function showVideoAd(tag, finish) {
        if (!AF.platform.isWxApp() && !AF.platform.isQQApp()) {
            if (AF.Random.getChance(50)) {
                finish && finish(true);
            } else {
    ...
    //Banner广告
    showBannerAd: function showBannerAd(tag) {
    ...
    getRankList: function getRankList(cb) {
        if (AF.platform.isWxApp()) {
            ...
            var nickName = ['送舟行', '狂奔蜗牛', '献世礼', '庸颜', '违心', '↘锁芯ラ', '枝桠', '实习生', '半衾梦', '舟遥客', '書生', '满栀', '遮云壑', '枕花眠', '暗中', '盗琴', '佞臣', '世佛', '孤凫', '三生路', '赠佳期'];
            var rankList = [];
            var length = Math.ceil(20 * Math.random());
            var selfFlag = Math.floor(length * Math.random());
    ...
    setButtonInteractable: function setButtonInteractable(node, flag, specialButton) {
    ...
    checkShareMsg: function checkShareMsg(msg) {
    ...
    getText: function getText(key, opt, defaultStr) {
        if (!polyInst) {
            initPolyglot(AF.GameConfig.getTextData());
        }

        var has = polyInst.has(key);
        if (!has) {
    ...
    requestServerApi: function requestServerApi(api, data, success, fail) {
        api = api || "";
        data = data || {};
    ...
    getMoveTime: function getMoveTime(distance, velocity, acceleration) {
    ...
    getDestination: function getDestination(posY, velocity, acceleration, time) {
    ...
    getShareUserInfo: function getShareUserInfo(cb) {
    ...
    getMusicUrl: function getMusicUrl(filename) {
    ...
    getSoundUrl: function getSoundUrl(filename) {
    ...
    isSoundExist: function isSoundExist(filename) {
        var name = "sfx/sound/" + filename + ".mp3";

        return AF.util.isLocalFileExist(name);
    },

    getRemoteUrl: function getRemoteUrl(filename) {
        var url = AF.platform.getCdnUrl() + REMOTE_DIR + filename;
        url += "?ts=" + new Date().getTime();
        return url;
    },

    isLocalFileExist: function isLocalFileExist(filename) {
        return AF.util.isLocalFileExist2(REMOTE_DIR + filename);
    },

    isLocalFileExist2: function isLocalFileExist2(filename) {
    ...
    getLocalFilePath: function getLocalFilePath(filename) {
        return AF.util.getLocalFilePath2(REMOTE_DIR + filename);
    },

    getLocalFilePath2: function getLocalFilePath2(filename) {
        if (AF.platform.isWxApp()) {
    ...
    makesureLocalPath: function makesureLocalPath(filename) {
        filename = REMOTE_DIR + filename;
        return AF.util.makesureLocalPath2(filename);
    },

    makesureLocalPath2: function makesureLocalPath2(filename) {
    ...
    saveLocalFile: function saveLocalFile(srcFile, dstFile) {
        if (AF.platform.isWxApp()) {
            var fs = wx.getFileSystemManager();
    ...
    copyLocalFile: function copyLocalFile(srcFile, dstFile) {
        if (AF.platform.isWxApp()) {
            var fs = wx.getFileSystemManager();
    ...
    checkShareGift: function checkShareGift() {
        var shareInfo = AF.util.getShareInfo();
...
cc._RF.pop();
        ...
        //# sourceMappingURL=af-utils.js.map
